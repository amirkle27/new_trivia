...
$$
BEGIN
    -- Insert or update the answer in player_answers
    ...
    -- Increment the questions_solved column in players
    UPDATE players
    SET questions_solved = COALESCE(questions_solved, 0) + 1
    WHERE player_id = p_player_id;

    -- Increment the player's score in players if the answer is correct
    IF p_is_correct THEN
        UPDATE players
        SET score = COALESCE(score, 0) + 1
        WHERE player_id = p_player_id;

        -- Also increment the player's score in high_scores for the current game
        UPDATE high_scores
        SET score = COALESCE(score, 0) + 1
        WHERE player_id = p_player_id
...
$$
BEGIN
    -- Insert a new entry in the high_scores table for the new game
    INSERT INTO high_scores (player_id, started_at)
    VALUES (p_player_id, CURRENT_TIMESTAMP);
END;
$$;
...
$$
BEGIN
    -- If a game is finished, return best game statistics
    RETURN QUERY
    SELECT
        hs.player_id,
        p.username,
        ...
        hs.finished_at,
        hs.total_game_time,
        NULL::INT AS current_question_id, -- No mid-game details for finished games
    ...
    -- Fallback: If no game is finished, show mid-game statistics
    IF NOT FOUND THEN
        RETURN QUERY
        SELECT
            p.player_id,
            ...
            hs.started_at,
            NULL::TIMESTAMP AS finished_at, -- Game not finished
        ...
        LIMIT 1; -- Limit to one record to show mid-game status
    END IF;
END;
$$;
...
BEGIN
    RETURN QUERY
        SELECT
            p.player_id,
            p.username,
            -- Count of all answered questions
            (SELECT COUNT(*)::INTEGER
             FROM player_answers pa
             WHERE pa.player_id = p.player_id) AS answered_questions,
            -- Count of all correct answers
            (SELECT COUNT(*)::INTEGER
             FROM player_answers pa
            ...
            pa.selected_answer,
            pa.is_correct,
            -- Calculate elapsed time with fallback
            (CASE
...
$$
BEGIN
    -- Fetch from high_scores if available
    RETURN QUERY
    SELECT hs.player_id,
           p.username,
           p.questions_solved,
    ...
    -- Fallback: Calculate dynamically if no entries in high_scores
    IF NOT FOUND THEN
        RETURN QUERY
        SELECT p.player_id,
               p.username,
               ...
               NULL::TIMESTAMP AS finished_at,  -- Explicitly cast NULL
